...
sql = """select a.date, ifnull(a.average,0) as data1, ifnull(b.average,0) as data2, ifnull(c.average,0) as data3, ifnull(d.average,0) as data4 from 
(SELECT date(timestamp) as date, Label, EXP( SUM( LOG( value ) ) / COUNT( * ) ) as average FROM `dump80` where Label = 'O3 hr' group by date(timestamp) order by date(timestamp) asc)a 
left join 
(SELECT date(timestamp) as date, Label, EXP( SUM( LOG( value ) ) / COUNT( * ) ) as average FROM `dump80` where Label = 'O3 hr' group by date(timestamp) order by date(timestamp) asc) b 
on a.date = b.date
left join 
(SELECT date(timestamp) as date, Label, EXP( SUM( LOG( value ) ) / COUNT( * ) ) as average FROM `dump80` where Label = 'NOx hr' group by date(timestamp) order by date(timestamp) asc) c 
on a.date = c.date
left join 
(SELECT date(timestamp) as date, Label, EXP( SUM( LOG( value ) ) / COUNT( * ) ) as average FROM `dump80` where Label = 'NOx hr' group by date(timestamp) order by date(timestamp) asc) d
on a.date = d.date
order by a.date asc"""
